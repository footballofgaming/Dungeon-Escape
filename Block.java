import greenfoot.*;
/**
 * Code for the block collision and block objects.
 * 
...
public class Block extends Actor {
    ...
    public Block (int x, int y, String m) {
        image = m;
    }

    public Block(int x, int y, boolean b, int e, int k){
        movable=b;
        move=b;
        event=e;
        keypos=k;
    ...
    public void collisionDetection()
    {
        ...
        //This checks if a block can be pushed into this tile (e.g. water or lava)
        Class[] moveOverTiles = {Water.class, Lava.class}; //Block can be pushed onto water or lava so the player can then go over it
        while (collisionAmount<moveOverTiles.length){
            Actor object = getOneObjectAtOffset(0,0,moveOverTiles[collisionAmount]);
            if (object!=null){
                Greenfoot.playSound("Steam.mp3");
                getWorld().removeObject(object); //removes the object underneath the block
                getWorld().removeObject(this); //removes the block itself so the player is not stopped by it
                return;
            }
            //prevents the program from getting angry once the object is removed
            collisionAmount++;
        }
